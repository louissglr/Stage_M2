...
# Définir les seuils de QC
seuils_qc <- list(
  "nCount_RNA_min"       = list(var = "nCount_RNA", op = "<",  seuil = 50),
  "nCount_RNA_max"       = list(var = "nCount_RNA", op = ">",  seuil = 7000),
  "nFeature_RNA_min"     = list(var = "nFeature_RNA", op = "<", seuil = 200),
  "nFeature_RNA_max"     = list(var = "nFeature_RNA", op = ">", seuil = 5000),
  "percent.mt_max"       = list(var = "percent.mt", op = ">",  seuil = 15),
  "doublets.score_max"   = list(var = "doublets.score", op = ">", seuil = 0.6),
  "log10GenesPerUMI_min" = list(var = "log10GenesPerUMI", op = "<", seuil = 0.8)
)

# Initialiser une liste pour stocker les résultats
resultats_filtrage <- lapply(names(seuils_qc), function(nom) {
  critere <- seuils_qc[[nom]]
  donnees_var <- seurat_object@meta.data[[critere$var]]
  
  # Appliquer le filtre selon l'opérateur
  nb_filtrees <- switch(critere$op,
                        "<" = sum(donnees_var < critere$seuil, na.rm = TRUE),
                        ">" = sum(donnees_var > critere$seuil, na.rm = TRUE)
  )
  
  # Retourner une ligne avec le nom du filtre et le nombre de cellules filtrées
  data.frame(
    filtre = nom,
    seuil = critere$seuil,
    cellules_filtrees = nb_filtrees
  )
})

# Combiner les résultats en un seul dataframe
df_filtrage <- bind_rows(resultats_filtrage)
