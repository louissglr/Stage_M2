...
ggsave(
  filename = file.path(umap_output_dir, "seurat_umap_origident.png"),
  plot = p,
  bg = "black",
  width = 7,
  height = 6,
...
p <- DimPlot(seurat, reduction = "umap", group.by = "seurat_clusters", label = T) +
  theme_minimal() +
  theme(
    panel.background = element_rect(fill = "black", color = NA),
    plot.background = element_rect(fill = "black", color = NA),
...
# Sauvegarder le graphique
ggsave(
  filename = file.path(umap_output_dir, "seurat_umap_clusters.png"),
  plot = p,
  bg = "black",
  width = 7,
  height = 6,
