...
barplot1 <- ggplot(df_pattern, aes(x = factor(pattern_cogaps), y = n, fill = seurat_clusters,
  ...
  geom_text(aes(y = pos_n, label = label), color = "white", size = 5, fontface = "bold", na.rm = TRUE) +
  geom_text(data = totals_pattern, aes(x = pattern_cogaps, y = total_cells + 50,
                                       label = paste0("n = ", total_cells)),
            inherit.aes = FALSE, size = 4, hjust = 0, fontface = "plain", color = "white") +
  coord_flip(clip = "off") +
  ylim(0, 2300) +
...
ggsave("C:/Users/louis/Desktop/Stage/scrnaseq_final/script/Plot_Diapo_Black/panel_ABC_patchwork_black.png",
       combined_plot, width = 18, height = 6, dpi = 300, bg = "black")
