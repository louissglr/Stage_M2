...
# Fonction pour ajouter le nombre de cellules juste au-dessus du maximum
...
# === Violin plot par Seurat clusters ===
p1 <- VlnPlot(
  object = seurat_obj,
...
# Étendre l’axe Y pour p1
p1 <- p1 +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(NA, max_y1 * 1.15)) +
  scale_x_discrete(labels = function(x) gsub("Pattern_", "", x))  # <-- ligne ajoutée

# Ajouter le nombre de cellules
p1 <- add_cell_count(p1, seurat_obj@meta.data, "pattern_cogaps")

# Sauvegarder en PNG
ggsave("C:/Users/louis/Desktop/Stage/scrnaseq_final/output/13_SignatureImSigPattern/ImSig_ViolinPlot_CoGAPSPatterns.png", plot = p1, width = 6, height = 6, dpi = 300, bg = "black")
